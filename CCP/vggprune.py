...
parser.add_argument('--save', default='./pvgg5', type=str, metavar='PATH',
...
for k, m in enumerate(model.modules()):
    ...
    if isinstance(m, nn.BatchNorm2d):
        ...
        mask = weight_copy.gt(thre).float().cuda()
        if torch.sum(mask) == 0:
            cfg.append(len(weight_copy))
            cfg_mask.append(torch.ones(len(weight_copy)))
            i +=1
            print('layer index: {:d} \t total channel: {:d} \t remaining channel: {:d}'.
                format(k, len(weight_copy), int(len(weight_copy))))
        else:
...
savepath = os.path.join(args.save, "prune_0.1_27.txt")
...
torch.save({'cfg': cfg, 'state_dict': newmodel.state_dict()}, os.path.join(args.save, 'pruned.pth_0.1_27.tar'))
